...
try {
    ...
    if (!$category) die("Catégorie Node.js non trouvée.");
    ...
    ');
    $offersStmt->execute(['nodejs']);
    $offers = $offersStmt->fetchAll();
    
} catch (PDOException $e) {
    error_log('NodeJS offers error: ' . $e->getMessage());
    ...
    <header class="text-center py-20 px-6 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <div class="inline-flex items-center gap-2 bg-green-500/10 text-green-400 border border-green-500/20 px-4 py-1.5 rounded-full text-xs font-semibold mb-5">
                <i class="<?php echo htmlspecialchars($category['icon'] ?? 'fab fa-node-js'); ?>"></i>
                Hébergement Web
            </div>
            ...
            <p class="text-gray-400 max-w-2xl mx-auto text-lg">
                Serveurs Node.js performants pour vos applications temps réel
            </p>
            <div class="mt-8">
