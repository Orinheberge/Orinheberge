...
try {
    ...
    if (!$category) die("Catégorie PHP non trouvée.");
    ...
    ');
    $offersStmt->execute(['php']);
    $offers = $offersStmt->fetchAll();
    
} catch (PDOException $e) {
    error_log('PHP offers error: ' . $e->getMessage());
...
<head>
    ...
    <style>
        ...
        .gradient-text { background: linear-gradient(90deg, #a855f7, #7c3aed); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    ...
    <header class="text-center py-20 px-6 relative overflow-hidden">
        ...
        <div class="relative z-10">
            <div class="inline-flex items-center gap-2 bg-purple-500/10 text-purple-400 border border-purple-500/20 px-4 py-1.5 rounded-full text-xs font-semibold mb-5">
                <i class="<?php echo htmlspecialchars($category['icon'] ?? 'fab fa-php'); ?>"></i>
                Hébergement Web
            </div>
            ...
            <p class="text-gray-400 max-w-2xl mx-auto text-lg">
                Hébergement PHP optimisé avec les dernières versions et extensions
            </p>
            <div class="mt-8">
