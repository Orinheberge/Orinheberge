...
try {
    ...
    if (!$category) die("Catégorie Java non trouvée.");
    ...
    ');
    $offersStmt->execute(['java']);
    $offers = $offersStmt->fetchAll();
    
} catch (PDOException $e) {
    error_log('Java offers error: ' . $e->getMessage());
...
<head>
    ...
    <style>
        ...
        .gradient-text { background: linear-gradient(90deg, #ef4444, #f97316); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
...
<main class="flex-grow">
    <header class="text-center py-20 px-6 relative overflow-hidden">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-red-500/10 rounded-full blur-[150px] pointer-events-none"></div>
        <div class="relative z-10">
            <div class="inline-flex items-center gap-2 bg-red-500/10 text-red-400 border border-red-500/20 px-4 py-1.5 rounded-full text-xs font-semibold mb-5">
                <i class="<?php echo htmlspecialchars($category['icon'] ?? 'fab fa-java'); ?>"></i>
                Hébergement Web
            </div>
            ...
            <p class="text-gray-400 max-w-2xl mx-auto text-lg">
                Hébergement Java pour Spring Boot, applications et serveurs
            </p>
            <div class="mt-8">
