...
try {
    ...
    if (!$category) die("Catégorie Azuriom non trouvée.");
    ...
    ');
    $offersStmt->execute(['azuriom']);
    $offers = $offersStmt->fetchAll();
    
} catch (PDOException $e) {
    error_log('Azuriom offers error: ' . $e->getMessage());
...
<main class="flex-grow">
    <header class="text-center py-20 px-6 relative overflow-hidden">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-blue-500/10 rounded-full blur-[150px] pointer-events-none"></div>
        <div class="relative z-10">
            <div class="inline-flex items-center gap-2 bg-blue-500/10 text-blue-400 border border-blue-500/20 px-4 py-1.5 rounded-full text-xs font-semibold mb-5">
                <i class="<?php echo htmlspecialchars($category['icon'] ?? 'fas fa-globe'); ?>"></i>
                Hébergement Web
            </div>
            ...
            <p class="text-gray-400 max-w-2xl mx-auto text-lg">
                Hébergement optimisé pour le CMS Azuriom
            </p>
            <div class="mt-8">
