...
try {
    ...
    if (!$category) die("Catégorie Python non trouvée.");
    ...
    ');
    $offersStmt->execute(['python']);
    $offers = $offersStmt->fetchAll();
    
} catch (PDOException $e) {
    error_log('Python offers error: ' . $e->getMessage());
...
<head>
    ...
    <style>
        ...
        .gradient-text { background: linear-gradient(90deg, #eab308, #3b82f6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
...
<main class="flex-grow">
    <header class="text-center py-20 px-6 relative overflow-hidden">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-yellow-500/10 rounded-full blur-[150px] pointer-events-none"></div>
        <div class="relative z-10">
            <div class="inline-flex items-center gap-2 bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 px-4 py-1.5 rounded-full text-xs font-semibold mb-5">
                <i class="<?php echo htmlspecialchars($category['icon'] ?? 'fab fa-python'); ?>"></i>
                Hébergement Web
            </div>
            ...
            <p class="text-gray-400 max-w-2xl mx-auto text-lg">
                Hébergement Python pour Django, Flask et vos bots
            </p>
            <div class="mt-8">
                <a href="/offres/web/" class="inline-flex items-center gap-2 text-gray-400 hover:text-white transition">
                    <i class="fas fa-arrow-left"></i> Retour aux offres Web
                </a>
            </div>
        </div>
    </header>
