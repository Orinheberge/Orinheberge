...
try {
    $stmt = $pdo->prepare('
        ...
        LIMIT 1
    ');
    $stmt->execute(['terraria']);
    $category = $stmt->fetch();
    
    if (!$category) die("Catégorie Terraria non trouvée.");
    ...
    ');
    $offersStmt->execute(['terraria']);
    $offers = $offersStmt->fetchAll();
    
} catch (PDOException $e) {
    error_log('Terraria error: ' . $e->getMessage());
...
<head>
    ...
    <style>
        ...
        .gradient-text { background: linear-gradient(90deg, #06b6d4, #0891b2); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
...
<main class="flex-grow">
    <header class="text-center py-20 px-6 relative overflow-hidden">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-cyan-500/10 rounded-full blur-[150px] pointer-events-none"></div>
        <div class="relative z-10">
            <div class="inline-flex items-center gap-2 bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 px-4 py-1.5 rounded-full text-xs font-semibold mb-5">
                <i class="<?php echo htmlspecialchars($category['icon'] ?? 'fas fa-tree'); ?>"></i>
            ...
            <p class="text-gray-400 max-w-2xl mx-auto text-lg">
                <?php echo t($category['description_key'] ?? 'Serveurs Terraria pour vos aventures 2D'); ?>
            </p>
            <div class="mt-8">
