...
<?php
try {
    $pdo2 = new PDO('mysql:host=localhost;dbname=s43_orinheberge;charset=utf8mb4','root','1504',[PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION,PDO::ATTR_DEFAULT_FETCH_MODE=>PDO::FETCH_ASSOC,PDO::ATTR_TIMEOUT=>3]);
    $db_status = true;
    if ($is_logged_in) {
        $u = $pdo2->prepare('SELECT pseudo,firstname,avatar FROM users WHERE id=? LIMIT 1');
        $u->execute([$_SESSION['user_id']]);
        $ud = $u->fetch();
        if ($ud) { $_SESSION['username']=!empty($ud['pseudo'])?$ud['pseudo']:$ud['firstname']; $_SESSION['avatar']=$ud['avatar']; }
    }
    $cq = $pdo2->query('SELECT DISTINCT category_slug,name_key,icon,image_url FROM categories_products WHERE is_active=1 GROUP BY category_slug ORDER BY sort_order ASC');
    while ($r = $cq->fetch()) $dynamic_categories[$r['category_slug']] = ['name_key'=>$r['name_key'],'icon'=>$r['icon'],'image_url'=>$r['image_url']];
    $stmt = $pdo2->query("SELECT p.*,cp.category_slug,cp.icon AS cat_icon,cp.image_url AS cat_image FROM categories_products cp JOIN products p ON p.id=cp.product_id WHERE cp.is_active=1 AND p.is_active=1 ORDER BY p.sort_order,p.id");
    foreach ($stmt->fetchAll() as $pr) {
        $slug=$pr['slug'];$cat=strtolower($pr['category_slug']);
        $tier=strpos($slug,'free')!==false?'free':(strpos($slug,'basic')!==false?'basic':(strpos($slug,'medium')!==false?'medium':'premium'));
        ...
        $sections[$tier]['offers'][]=['category'=>$cat,'slug'=>$slug,'name'=>$pr['name'],'desc'=>$pr['description']??'','price'=>$pr['type']==='free'?'0€':number_format($pr['price'],2,',','').'€','price_value'=>(float)$pr['price'],'period_key'=>$pr['type']==='free'?'offers.period.free':'offers.period.month','free'=>$pr['type']==='free','icon'=>$pr['cat_icon']??'fas fa-server','image_url'=>$pr['cat_image']??'','features'=>[['icon'=>'fas fa-memory','text'=>$rt],['icon'=>'fas fa-hard-drive','text'=>$dt],['icon'=>'fas fa-microchip','text'=>$pr['cpu'].'% CPU'],['icon'=>'fas fa-database','text'=>$pr['databases'].' BDD']]];
    }
} catch(PDOException $e){}

function tierStyle($t){return['free'=>['bb'=>'bg-green-500/20','bt'=>'text-green-400','bbd'=>'border-green-500/30','ic'=>'text-green-400','cb'=>'border-white/10','btn'=>'bg-green-500 hover:bg-green-400'],'basic'=>['bb'=>'bg-blue-500/20','bt'=>'text-blue-400','bbd'=>'border-blue-500/30','ic'=>'text-blue-400','cb'=>'border-blue-400/20','btn'=>'bg-blue-500 hover:bg-blue-400'],'medium'=>['bb'=>'bg-purple-500/20','bt'=>'text-purple-400','bbd'=>'border-purple-500/30','ic'=>'text-purple-400','cb'=>'border-purple-400/20','btn'=>'bg-purple-500 hover:bg-purple-400'],'premium'=>['bb'=>'bg-yellow-500/20','bt'=>'text-yellow-400','bbd'=>'border-yellow-500/30','ic'=>'text-yellow-400','cb'=>'border-yellow-400/20','btn'=>'bg-yellow-500 hover:bg-yellow-400']][$t]??['bb'=>'bg-gray-500/20','bt'=>'text-gray-400','bbd'=>'border-gray-500/30','ic'=>'text-gray-400','cb'=>'border-white/10','btn'=>'bg-gray-500 hover:bg-gray-400'];}
?>
...
<main class="flex-grow">
  ...
  <header class="text-center py-16 px-6 relative overflow-hidden">
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-sky-500/8 rounded-full blur-[120px] pointer-events-none"></div>
  ...
  <section id="cat-view" class="py-16 px-6">
    <div class="text-center mb-12">
      ...
      <p class="text-gray-400 mt-4 text-sm">Toutes les offres disponibles, du moins cher au plus cher.</p>
    </div>
    <div class="max-w-7xl mx-auto cat-grid" id="cat-view-grid"></div>
  </section>

  <!-- Vue toutes les offres par tiers -->
  <div id="all-sections">
  <?php foreach($sections as $tier_key => $tier): if(empty($tier['offers'])) continue; $s=tierStyle($tier_key); ?>
    ...
    <div data-category="<?php echo htmlspecialchars($offer['category']); ?>" data-price="<?php echo $price_num; ?>"
      ...
      <div class="p-5 flex flex-col flex-grow">
        ...
        <form method="post" action="/shop/cart/">
          ...
          <button type="submit" class="w-full <?php echo $s['btn']; ?> text-slate-950 font-bold py-2.5 rounded-xl text-sm"><?php echo $btn_text; ?></button>
        </form>
        <?php else: ?>
        <a href="/login/" class="w-full <?php echo $s['btn']; ?> text-slate-950 font-bold py-2.5 rounded-xl text-sm text-center block"><?php echo $btn_text; ?></a>
        <?php endif; ?>
      </div>
    </div>
    <?php endforeach; ?>
...
<div class="fixed bottom-6 right-6 z-50">
  <a href="/discord/" target="_blank" class="bg-[#5865F2] hover:bg-[#4752C4] text-white px-5 py-3.5 rounded-full font-bold flex items-center gap-2 shadow-2xl hover:scale-105 transform duration-200 transition">
    <i class="fab fa-discord text-xl"></i><span class="hidden sm:inline text-sm"><?php echo t('discord.help'); ?></span>
  </a>
</div>
