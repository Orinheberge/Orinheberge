...
// Clé d'encryption pour les cartes (legacy)
...
// ── Fonctions encryption (legacy cards) ──
...
// Vérifier si Discord/Google liés
$discordLinked = false;
$googleLinked = false;
...
<body class="min-h-screen text-gray-300 font-sans flex flex-col">
        ...
        <main class="p-6 lg:p-10 w-full max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div class="lg:col-span-3 space-y-6 pb-10">
                    ...
                    <!-- ═══════════════════════════════════════════ -->
                    <!-- SECTION 3 : COMPTES CONNECTÉS (OAuth) -->
                    <!-- ═══════════════════════════════════════════ -->
                    ...
                    <!-- ═══════════════════════════════════════════ -->
                    <!-- SECTION 4 : MOYENS DE PAIEMENT (Stripe) -->
                    <!-- ═══════════════════════════════════════════ -->
                    <?php if ($stripe_enabled): ?>
    ...
    document.getElementById('payment-form').addEventListener('submit', async function(e) {
        ...
        try {
            const resp = await fetch('/client/cards/create-intent.php', {
                method: 'POST', 
                headers: {'Content-Type': 'application/json'}
            });
            ...
            if (saveData.success) {
                window.location.href = '/profil/?success=1#payment';
            } else {
