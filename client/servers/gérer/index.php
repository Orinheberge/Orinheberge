...
// Constantes utilisées par callPterodactylClientAPI()
if (!defined('PTERODACTYL_URL'))            define('PTERODACTYL_URL', $panel_url);
if (!defined('PTERODACTYL_CLIENT_API_KEY')) define('PTERODACTYL_CLIENT_API_KEY', $api_key_client);
...
function callPterodactylClientAPI($endpoint, $method = 'GET', $data = null, $customHeaders = [], $customApiKey = null) {
    $url = rtrim(PTERODACTYL_URL, '/') . '/api/client/' . $endpoint;

    $apiKey = $customApiKey ?: PTERODACTYL_CLIENT_API_KEY;
    // Headers de base
    $headers = [
        'Authorization: Bearer ' . $apiKey,
        'Accept: application/json'
    ];
    // Fusion avec d'éventuels headers personnalisés (ex: Content-Type: text/plain)
    ...
    if ($data !== null) {
        // Si les données sont un tableau, on convertit en JSON, sinon on transmet tel quel (ex: texte brut de fichier)
        curl_setopt($ch, CURLOPT_POSTFIELDS, is_array($data) ? json_encode($data) : $data);
    }
    $response = curl_exec($ch);
    $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    curl_close($ch);
    // Tentative de décodage JSON, si échec (ex: fichier texte brut), on conserve la réponse brute
    $decodedData = json_decode($response, true);
    return [
        'code' => $httpCode,
        'data' => ($decodedData !== null) ? $decodedData : $response
    ];
}
...
if (isset($_GET['get_ws_token'])) {
    ...
    $wsResponse = callPterodactylClientAPI($endpoint);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_power'])) {
    ...
    if (in_array($signal, ['start', 'stop', 'restart', 'kill'])) {
        $res = callPterodactylClientAPI("servers/$short_identifier/power", "POST", ["signal" => $signal]);
...
$details = callPterodactylClientAPI("servers/$short_identifier");
$ram_max = 0;
$disk_max = 0;
