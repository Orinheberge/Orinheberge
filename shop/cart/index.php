...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_item') {
        ...
        try {
            ...
            // On ne calcule PAS le total et on ne crée PAS la commande ici :
            // /shop/order/ s'en charge déjà entièrement (bundle, offres
            // gratuites, code promo, sélection de node, écriture dans
            // `orders`, facture, webhook Discord) et sait lire les
            // quantités du panier via $_SESSION['checkout_bundle']['items']
            // (fonction findSlugQuantity() de ce script).
            $_SESSION['checkout_bundle'] = [
                'items' => array_map(static function (array $bi): array {
                    return ['slug' => $bi['slug'], 'quantity' => $bi['quantity']];
                }, $bundle_items),
            ];

            $target_url = '/shop/order/?plan=' . urlencode(implode(',', $bundle_slugs));
