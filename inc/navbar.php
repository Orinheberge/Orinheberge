...
<nav class="sticky top-0 z-50 glass p-5 border-b border-white/5">
    <div class="max-w-7xl mx-auto flex justify-between items-center gap-4">
        ...
        <div class="hidden md:flex items-center gap-2 lg:gap-3 whitespace-nowrap">
            <a href="/" class="<?php echo $active_nav === 'home' ? 'bg-sky-600/30 text-sky-400 border-sky-500/50 font-bold' : 'bg-sky-600/5 text-sky-400/70 hover:text-sky-300 border-sky-500/10 hover:bg-sky-600/20'; ?> px-4 py-2 rounded-full text-xs flex items-center gap-2 transition font-medium shadow-md border">
                <i class="fas fa-home"></i> <?php echo t('nav.home'); ?>
            </a>
            
            <a href="/client/servers/" class="<?php echo $active_nav === 'servers' ? 'bg-slate-600/40 text-slate-300 border-slate-500/60 font-bold' : 'bg-slate-600/10 text-slate-400 hover:text-slate-200 border-slate-500/15 hover:bg-slate-600/30'; ?> px-4 py-2 rounded-full text-xs flex items-center gap-2 transition font-medium shadow-md border">
                <i class="fas fa-server"></i> <?php echo t('nav.servers'); ?>
            </a>
            
            <a href="/shop/" class="<?php echo $active_nav === 'offers' ? 'bg-amber-600/30 text-amber-400 border-amber-500/50 font-bold' : 'bg-amber-600/5 text-amber-400/70 hover:text-amber-300 border-amber-500/10 hover:bg-amber-600/20'; ?> px-4 py-2 rounded-full text-xs flex items-center gap-2 transition font-medium shadow-md border">
                <i class="fas fa-tags"></i> <?php echo t('nav.offers'); ?>
            </a>
            
            <a href="/support/" class="<?php echo $active_nav === 'support' ? 'bg-purple-600/30 text-purple-400 border-purple-500/50 font-bold' : 'bg-purple-600/5 text-purple-400/70 hover:text-purple-300 border-purple-500/10 hover:bg-purple-600/20'; ?> px-4 py-2 rounded-full text-xs flex items-center gap-2 transition font-medium shadow-md border">
            ...
            <?php if (isset($_SESSION['user_id'])): ?>
                ...
                <a href="/logout/" class="bg-red-600/10 hover:bg-red-600/20 border border-red-500/20 text-red-400 hover:text-red-300 px-4 py-2 rounded-full text-xs flex items-center gap-2 transition font-medium border-white/5 shadow-md shadow-red-950/10">
                    <i class="fas fa-sign-out-alt"></i> <?php echo t('nav.logout'); ?>
                </a>
            <?php else: ?>
                <a href="/login/" class="bg-slate-600/10 border border-slate-500/20 text-slate-400 hover:text-white px-4 py-2 rounded-full text-xs flex items-center gap-2 transition font-medium">
                    <i class="fas fa-sign-in-alt"></i> <?php echo t('nav.login'); ?>
                </a>
                <a href="/register/" class="bg-slate-600/10 border border-slate-500/20 text-slate-400 hover:text-white px-4 py-2 rounded-full text-xs flex items-center gap-2 transition font-medium">
                    <i class="fas fa-user-plus"></i> <?php echo t('nav.register'); ?>
                </a>
            <?php endif; ?>
        </div>

        <div class="hidden md:flex gap-2.5 items-center shrink-0">
    ...
    <div id="mobileMenu" class="md:hidden mt-4 px-4 space-y-3 glass rounded-2xl p-4 hidden">
        ...
        <?php if (isset($_SESSION['is_admin']) && $_SESSION['is_admin']): ?>
            <a href="/support/admin_tickets/" class="block py-2 px-4 rounded-xl bg-rose-600/20 border border-rose-500/30 text-rose-400 flex items-center gap-2 text-sm font-semibold">
